...
const Order = () => {
  ...
  useEffect(() => {
    ...
    setUserId(decoded.result.id)
     axios.get(`http://localhost:8080/ecom-api/orders/user/${decoded.result.id}`,config)
    .then(response =>{
  ...
  return (
    ...
    <Container>
      ...
      {order.map(item=>(
       ...
       <p style={{marginLeft:'80px',backgroundColor:'red',width:'8px',height:'8px',borderRadius:'50%'}}></p>
       <small style={{color:'black',marginLeft:'10px',fontSize:'14px',marginTop:'12px'}}> Shipped {item.date}</small>
       </div>
       <ImageArea>
       <Image src='data:image/jpeg;base64,/9j/4AAQSkZJRgABAQAAAQABAAD/2wCEAAo
       ...
       <Link href='/order/[id]' as={`/order/${item.id}`}><P>VIEW ORDER  ={'>'}</P></Link>
    </UserDiv>   
      ))}
